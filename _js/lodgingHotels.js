function lodgingHotels(_parent,_Name,_Pic_Url,_Accessibility,_Accommodation_Type,_Address,_City,_Hotel_Classification,_Kosher,_Near_To,_Number_of_Rooms,_Parking,_Rated,_Swimming_Pool,_Region,_URL,_WiFi,_X,_Y){
        this.parent=_parent;   
        this.name=_Name;
    ...
    lodgingHotels.prototype.addToHTML=function(){
        var newBox=document.createElement("div");
        newBox.className="boxLodgingHotels";
        this.parent.appendChild(newBox);
        var newBox1=document.createElement("div");
        newBox1.className="boxLodgingHotelsImg";
        newBox.appendChild(newBox1);
        newBox1.innerHTML="<img class='boxLodgingHotelsImgMore' src='"+this.imgFolder+this.pic_Url+"'>";
        var newBox2=document.createElement("div");
        newBox2.className="boxLodgingHotelsText";
            ...
            for (var i = 1; i <= 5; i++) {
                ...
                if (index != -1) {
                    ...
                    for (var j = 0; j < subStr; j++) {
                        newBox2.innerHTML+="<span class='boxLodgingMainHotelsClassification' style='font-size:280%;color:yellow;'>&starf;</span>";   
                    }
                    newBox2.innerHTML+="<strong class='boxLodgingHotelsName'>"+this.name+"</strong>";  
                }  
            }
             
        }
        else{
            newBox2.innerHTML+="<strong class='boxLodgingHotelsName'>"+this.name+"</strong><br>";
        }
        newBox2.innerHTML+="<p class='boxLodgingHotelsAddress'>"+this.address+"</p>";
        var name1 = this.name;
        this.distance(this.city,this.near_To,this.X,this.Y,this.name);
        if (localStorage[name1] != undefined) {
            if (localStorage[name1] <  1) {
                newBox2.innerHTML+="<div class='boxLodgingHotelsDistance'><span>י"+(localStorage[name1]*1000)+"</span><span> מטרים ממרכז העיר </span><img class='boxLodgingHotelsSymbolslocation' src='../_images/location.png'></div>";
            }
            else{
                newBox2.innerHTML+="<div class='boxLodgingHotelsDistance'><span>י"+(localStorage[name1])+"</span><span> קילומטרים ממרכז העיר </span><img class='boxLodgingHotelsSymbolslocation' src='../_images/location.png'></div>";
            }
        }
        if (this.WiFi != "לא") {
            newBox2.innerHTML+="<img class='boxLodgingHotelsSymbolsWifi' src='../_images/wifi.png'>";
        }
        if (this.parking != "לא") {
            newBox2.innerHTML+="<img class='boxLodgingHotelsSymbolsParking' src='../_images/parking.png'>";
        }
        if (this.accessibility != "לא") {
            newBox2.innerHTML+="<img class='boxLodgingHotelsSymbolsAccessibility' src='../_images/accessibility.png'>";
        }
    ...
    $('.addToFavorites').on('click', function() {
        ...
        if (test == false) {
        var alt = $(this).attr("alt");
        FindFavorites1(alt,"false","hotel"); 
        localStoragePage(0);
        }
    });
        newBox2.innerHTML+="<botton class='boxLodgingHotelsBtn'><span class='boxLodgingHotelsSpan'><a target='_blank' href='"+this.URL+"'<strong>למידע נוסף</strong></a></span></botton>";
    }
    lodgingHotels.prototype.distance=function(_cityFind,_nearTo,_x,_y,_name){
        $.ajax({
