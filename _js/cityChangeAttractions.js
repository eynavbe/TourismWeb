function CityChangeAttractions(_parent,_Name,_FullDescription,_Pic_Url,_Address,_Attraction_Type,_Opening_Hours,_Parking,_Suitable_for_Children,_URL){
    this.parent=_parent;
    this.name=_Name;
    this.fullDescription=_FullDescription;
    this.pic_Url=_Pic_Url;
    this.address=_Address;
    this.attraction_Type=_Attraction_Type;
    this.opening_Hours=_Opening_Hours;
    this.parking=_Parking;
    this.suitable_for_Children=_Suitable_for_Children;
    this.URL=_URL;
    this.imgFolder="https://motwebmediastg01.blob.core.windows.net/nop-thumbs-images/";
}
CityChangeAttractions.prototype.addToHTML=function(){
    
    var newBox=$("<div class='boxAttractionsCity card'>");
    $(this.parent).append(newBox);
    var newImg=$("<img class='boxAttractionsCityImg' src='"+this.imgFolder+this.pic_Url+"'>");
    $(newBox).append(newImg);
    $(newBox).append("<div class='bookmark1'><h5 class='bookmark1Text'>"+this.attraction_Type+"</h5></div>");
    $(newBox).append("<h3 class='boxAttractionsCityName'>"+this.name+"</h3>");
    ...
    $(newBox).on("click",function(){
        ...
        var name = this.name;
        var accommodation_Type = "אטרקציה";
        ...
        $(id_darkCityP).append("<strong>מתאים לילדים: </strong>"+this.suitable_for_Children+"</br>");
